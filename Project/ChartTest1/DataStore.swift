...
struct DataStore {
    ...
    lazy var firstDay: [StockData] = {
        ...
        result = rawData.filter( { $0.date.isInSameDay(date: firstDate) } )
    ...
    lazy var firstFiveDays: [StockData] = {
        ...
        guard let firstDate = rawData.first?.date else { return result }
        guard let deadline = Calendar.current.date(byAdding: .day, value: -5, to: firstDate)?.endOfDay else { return result }
    ...
    lazy var sixMonths: [StockData] = {
        ...
        guard let firstDate = rawData.first?.date else { return result }
        guard let deadline = Calendar.current.date(byAdding: .month, value: -6, to: firstDate)?.endOfDay else { return result }
        // Get every element whose date is before or on the same date as the deadline.
    ...
    lazy var year: [StockData] = {
        ...
        guard let firstDate = rawData.first?.date else { return result }
        guard let deadline = Calendar.current.date(byAdding: .year, value: -1, to: firstDate)?.endOfDay else { return result }
        // Get every element whose date is before or on the same date as the deadline.
    ...
    lazy var fiveYears: [StockData] = {
        ...
        guard let firstDate = rawData.first?.date else { return result }
        guard let deadline = Calendar.current.date(byAdding: .year, value: -5, to: firstDate)?.endOfDay else { return result }
        // Get every element whose date is before or on the same date as the deadline.
